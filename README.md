# Antigravity Recruiter — Intelligent Candidate Discovery & Ranking Engine

This repository contains the complete, production-grade source code for **Antigravity Recruiter**, a hybrid candidate search and ranking engine built for the Redrob AI Recruiter Challenge.
...
To run the ranker on the full pool of candidates:
```bash
python rank.py --candidates ./Dataset/candidates.jsonl --out ./team_submission.csv
```
...
```bash
python Dataset/validate_submission.py team_submission.csv
```
...
* `Dataset/`:
  - `candidates.jsonl`: Full candidate pool (100K candidates).
  - `honeypot_blacklist.json`: Extracted blacklist of the 94 honeypot candidate IDs.
